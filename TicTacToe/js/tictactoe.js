...
function placeXorO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        if (activePlayer === 'X') {
            ...
            //active player may only be 'x' or 'x' so if not 'x' it must be 'o'
        } 
        else {
...
function checkWinConditions() {
    ...
    //O 0, 1, 2 condition:
    else if (arrayIncludes('0O', '1O', '2O')) { drawWinLine(50, 3100, 558, 100); }
    //O 3, 4, 5 condition
    ...
    //O 1, 4, 7 condition:
    else if (arrayIncludes('1O', '4O', '7O')) { drawWinLine(50, 304, 558, 304); }
    //O 2, 5, 8 condition:
    ...
    //this functions checks if an array includes 3 strings. it is used to check for each win condition
...
function drawWinLine(coordX1, coordY1, coordX2, coordY2) {
    ...
    function animateLineDrawing() {
        ...
        c.stroke();
        //this condition check if we've reached the endpoints
        if (x1 <= x2 && y1 <= y2) {
